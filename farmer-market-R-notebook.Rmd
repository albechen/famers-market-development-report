...
fm_graph <- filter(fm_graph, total_goods!=0)
...
ggplot(data=fm_graph, aes(x=total_goods, fill=Region)) + 
  ...
  scale_fill_aaas() +
  ggtitle("Total Types of Goods of Farmers Markets by Region") +
  xlab("Total Types of Goods") + 
  ylab("")
...
food_join <- transform(food_join, rank = ave(Percent_No, Region, FUN = Rank))
food_join <- filter(food_join, rank<=10)
#food_join <- filter(food_join, Percent_Yes > 0.5)

ggplot(food_join, aes(area = Percent_Yes, 
                      fill=Region, 
                      label=goods, 
                      subgroup=Region)) +
  scale_fill_aaas() +
  ggtitle("Top 10 Majority Goods Available at Farmers Markets by Region") +
